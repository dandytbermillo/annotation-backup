...
import { FeaturePlan, FeatureStructure, FileTemplate } from './types';
import * as fs from 'fs-extra';
import * as path from 'path';

export class Scaffolder {
  private readonly baseDocsPath = 'docs/proposal';
  
  /**
   * Creates the complete feature structure
   */
  async createStructure(plan: FeaturePlan): Promise<FeatureStructure> {
   ...
   */
  async writeFiles(structure: FeatureStructure): Promise<number> {
    let count = 0;
   ...
   */
  private generateImplementationMd(plan: FeaturePlan): string {
    const lines: string[] = [
      `# ${plan.title}`,
      '',
   ...
   */
  private generateMainReport(plan: FeaturePlan): string {
...
[No fixes yet - feature in progress]`;
   ...
   */
  private generateFixesReadme(plan: FeaturePlan): string {
    const title = plan.title;
    const date = plan.date || new Date().toISOString().split('T')[0];
...
- **Root Cause Distribution**: TBD`;
   ...
   */
  private generateArtifactsIndex(): string {
...
4. Reference from main report`;
   ...
   */
  private generatePatchesReadme(): string {
