 ...
 * Tests all critical features as specified in the requirements
 * 
 * NOTE: For the most complete and correct test suite, consider using:
 * comprehensive-feature-test-corrected.js
 * 
 * This file has been updated with:
 * - Valid constraint values for type, table_name, entity_id
 * - Correct status flow (pending→processing→DELETE)
 * - Proper dead-letter fields (error_message, last_error_at)
 * 
...
const testSuites = {
    ...
    tests: [
      ...
      },
      {
        ...
        async test() {
          ...
          // Simulate successful processing by deleting the row (no 'completed' state)
          await pool.query('DELETE FROM offline_queue WHERE id = $1', [id]);
          const remaining = await pool.query('SELECT 1 FROM offline_queue WHERE id = $1', [id]);
          if (remaining.rowCount !== 0) {
            throw new Error('Processed item was not deleted');
          }
          return 'Status progression (pending→processing→delete) works';
        }
      },
      {
        ...
        async test() {
          ...
          await pool.query(
            `INSERT INTO offline_queue 
             ...
             VALUES 
             ($1, 'update', 'notes', $4, '{}'::jsonb, $2, $3, 'pending'),
             ($5, 'update', 'notes', $8, '{}'::jsonb, $6, $7, 'pending')`,
            [expiredId, crypto.randomUUID(), expired, crypto.randomUUID(),
             validId, crypto.randomUUID(), valid, crypto.randomUUID()]
          );
      ...
      },
      {
        ...
        async test() {
          ...
          // Move to dead letter (schema uses error_message and retry_count)
          await pool.query(
      ...
      },
      {
        ...
        async test() {
          ...
          if (res.data.skipped !== 1) {
            throw new Error('Duplicate not skipped');
          }
          
          return 'Duplicates skipped correctly';
        }
      }
    ]
  },
  ...
  fullTextSearch: {
    ...
    tests: [
        ...
        async test() {
          ...
          await pool.query(
            `INSERT INTO notes (id, title, metadata, created_at, updated_at)
             VALUES ($1, 'FTS Test Note', '{}'::jsonb, NOW(), NOW())
      ...
      {
        name: 'Trigram fuzzy search',
        async test() {
          // Test trigram similarity
          const result = await pool.query(
            `SELECT similarity('test', 'tset') as sim`
          );
    ...
    tests: [
      {
        ...
        async test() {
          ...
          await pool.query(
            `INSERT INTO notes (id, title, metadata, created_at, updated_at)
             VALUES ($1, 'Version Test Note', '{}'::jsonb, NOW(), NOW())
             ON CONFLICT (id) DO NOTHING`,
            [noteId]
          );
      ...
      },
      {
        ...
        async test() {
          ...
          await pool.query(
            `INSERT INTO notes (id, title, metadata, created_at, updated_at)
             VALUES ($1, 'Version Size Note', '{}'::jsonb, NOW(), NOW())
             ON CONFLICT (id) DO NOTHING`,
            [noteId]
          );
          await pool.query(
