 ...
 * Fixed version with proper UUIDs and table constraints
 */
...
const testSuites = {
    ...
    tests: [
      ...
      },
      {
        ...
        async test() {
          ...
          // Insert as pending (using valid table_name)
          await pool.query(
  ...
  apiContracts: {
    ...
    tests: [
      ...
      {
        name: 'Queue flush API with note_id',
        async test() {
          ...
          return 'Queue flush requires note_id';
        }
      }
    ]
  },
  ...
  fullTextSearch: {
    ...
    tests: [
        ...
        async test() {
          ...
          await pool.query(
            `INSERT INTO notes (id, title, content, metadata, created_at, updated_at)
             VALUES ($1, 'Test Note', '{}'::jsonb, '{}'::jsonb, NOW(), NOW())
      ...
      },
      {
        ...
        async test() {
          // Test trigram similarity with better match
          const result = await pool.query(
            `SELECT similarity('testing', 'tetsing') as sim`
          );
