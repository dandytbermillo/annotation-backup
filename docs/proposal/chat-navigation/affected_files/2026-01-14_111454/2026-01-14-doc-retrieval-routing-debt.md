...
**Date:** 2026-01-14
**Updated:** 2026-01-16
**Status:** Mostly Complete (6/7 items done)
...
- [x] Test table with 20+ common phrases (`__tests__/chat/query-patterns.test.ts` - 33+ cases)
- [x] CI runs pattern tests on every change (`npm test` runs jest)
...
**Acceptance Criteria:**
- [x] Monitor for this pattern in telemetry (TD-4)
- [x] If observed frequently, implement fix
- [x] "can you tell me more?" treated as follow-up
- [x] "can you explain what is workspace?" still treated as new question

**Implementation:** Modified `isPronounFollowUp()` in `lib/chat/query-patterns.ts` to use `stripConversationalPrefix()` before checking `FOLLOWUP_PHRASES`. "can you tell me more?" now strips to "more" which matches, while "can you explain what is workspace?" strips to "what is workspace" which doesn't match follow-up phrases.
...
| 6 | TD-5: Follow-up guard | Only if telemetry shows issue | ✅ Done |
