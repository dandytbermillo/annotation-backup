...
- Delete always requires confirmation (button/pill or explicit confirmation text).
...
- Resolve name match within current entry.
- If multiple matches, return selection list.
- If single match, return confirmation prompt.
- Only perform deletion after explicit confirmation.

## Confirmation Flow (Delete)
- Step 1: LLM returns intent "delete_workspace" with target name.
- Step 2: Resolver returns action "needs_confirmation" with confirm option.
- Step 3: User clicks confirm or types "confirm".
- Step 4: API deletes workspace and returns success message.
...
- delete_workspace requires confirmation and never deletes without it.
