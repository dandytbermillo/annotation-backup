'use client'

import { useState, useEffect } from 'react'
import { PlainModeProvider } from '@/app/providers/plain-mode-provider'

export default function TestPlainMode() {
  const [logs, setLogs] = useState<string[]>([])
  const [testNoteId, setTestNoteId] = useState<string | null>(null)
  
  useEffect(() => {
    // Ensure we're in plain mode
    localStorage.setItem('collab-mode', 'plain')
    
    // Create a test note on mount
    fetch('/api/postgres-offline/notes', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        title: 'Plain Mode Test Note',
        content: 'Test note for plain mode testing'
      })
    })
    .then(res => res.json())
    .then(data => {
      console.log('Created test note:', data.id)
      setTestNoteId(data.id)
      setLogs(prev => [...prev, `Test note created: ${data.id}`])
    })
    .catch(err => {
      console.error('Failed to create test note:', err)
      setLogs(prev => [...prev, `Error creating test note: ${err}`])
    })
    
    // Capture console logs
    const originalLog = console.log
    console.log = (...args) => {
      originalLog(...args)
      setLogs(prev => [...prev, args.join(' ')])
    }
    
    return () => {
      console.log = originalLog
    }
  }, [])
  
  const runTest = async () => {
    setLogs(['Starting test...'])
    
    if (!testNoteId) {
      setLogs(prev => [...prev, 'Error: Test note not created yet. Please wait and try again.'])
      return
    }
    
    // Test 1: Save a document using batch API
    try {
      const saveResponse = await fetch('/api/postgres-offline/documents/batch', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          operations: [{
            noteId: testNoteId,
            panelId: 'main',
            content: {
              type: 'doc',
              content: [
                {
                  type: 'paragraph',
                  content: [
                    {
                      type: 'text',
                      text: 'Test content saved at ' + new Date().toLocaleTimeString()
                    }
                  ]
                }
              ]
            }
          }]
        })
      })
      
      const saveResult = await saveResponse.json()
      setLogs(prev => [...prev, 'Save result: ' + JSON.stringify(saveResult)])
      
      // Test 2: Load the document
      const loadResponse = await fetch(`/api/postgres-offline/documents/${testNoteId}/main`)
      const loadResult = await loadResponse.json()
      setLogs(prev => [...prev, 'Load result: ' + JSON.stringify(loadResult, null, 2)])
      
    } catch (error) {
      setLogs(prev => [...prev, 'Error: ' + error])
    }
  }
  
  return (
    <PlainModeProvider>
      <div className="p-8">
        <h1 className="text-2xl font-bold mb-4">Plain Mode Test</h1>
        
        {/* Note Status */}
        <div className="mb-4">
          {!testNoteId ? (
            <div className="p-3 bg-yellow-100 border border-yellow-300 rounded">
              ⏳ Creating test note...
            </div>
          ) : (
            <div className="p-3 bg-green-100 border border-green-300 rounded">
              ✅ Test note ready: {testNoteId}
            </div>
          )}
        </div>
        
        <div className="mb-4">
          <button
            onClick={runTest}
            className="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600"
          >
            Run Test
          </button>
        </div>
        
        <div className="bg-gray-100 p-4 rounded">
          <h2 className="font-bold mb-2">Console Logs:</h2>
          <div className="font-mono text-xs whitespace-pre-wrap">
            {logs.map((log, i) => (
              <div key={i}>{log}</div>
            ))}
          </div>
        </div>
        
        <div className="mt-8">
          <h2 className="font-bold mb-2">Test Scenario:</h2>
          <ol className="list-decimal list-inside space-y-2">
            <li>Wait for "Test note ready" message to appear</li>
            <li>Click "Run Test" to save and load a document</li>
            <li>Check the console logs to see the batch API response</li>
            <li>Should show: processed: 1, skipped: 0 (or skipped: 1 if content unchanged)</li>
          </ol>
        </div>
      </div>
    </PlainModeProvider>
  )
}