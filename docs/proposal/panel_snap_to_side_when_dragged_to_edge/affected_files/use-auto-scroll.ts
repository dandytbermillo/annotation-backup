...
export const useAutoScroll = ({
  ...
  const checkAutoScroll = useCallback((clientX: number, clientY: number) => {
    ...
    if (shouldScroll) {
      ...
      debugLog('useAutoScroll', 'trigger_auto_scroll', {
        ...
        edges: edges.join('+'),
        distanceFromLeft: clientX,
        distanceFromRight: window.innerWidth - clientX,
        distanceFromTop: clientY,
        distanceFromBottom: window.innerHeight - clientY,
    ...
    setAutoScroll(prev => {
      if (
        ...
        prev.velocity.x === velocityX &&
        prev.velocity.y === velocityY
      ) {
        return prev
      }
  ...
  const stopAutoScroll = useCallback(() => {
    setAutoScroll(prev => {
      if (!prev.isActive && prev.velocity.x === 0 && prev.velocity.y === 0) {
        return prev
      }
