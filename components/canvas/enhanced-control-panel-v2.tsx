...
import { X, Settings, Lock, Unlock, Activity, Shield, Zap, Save, Download, Upload, RotateCcw, Eye, EyeOff, Maximize2, Minimize2, Bug } from 'lucide-react'
...
export function EnhancedControlPanelV2({ visible, onClose, canvasItems, onAddComponent }: EnhancedControlPanelV2Props) {
  ...
  const [activeTab, setActiveTab] = useState<'canvas' | 'isolation' | 'state' | 'debug'>('canvas')
  ...
  return (
    <div className="fixed top-20 right-4 bg-gray-900 border border-gray-700 rounded-lg shadow-2xl z-[1000] 
      ...
      <div className="flex border-b border-gray-700">
        {['canvas', 'isolation', 'state', 'debug'].map(tab => (
          <button
            key={tab}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-800 rounded p-3">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Current FPS:</span>
                ...
                <div className="flex justify-between">
                  ...
                  <span className={`font-mono font-bold ${
                    ...
                    {memory !== null ? `${memory}MB` : 'N/A'}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Isolated:</span>
                  <span className="font-mono font-bold text-white">{isolatedIds.length}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">System:</span>
                  <span className={`font-mono font-bold ${
                    isolationEnabled ? 'text-green-400' : 'text-gray-400'
                  }`}>
                    {isolationEnabled ? 'ON' : 'OFF'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-800 rounded p-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button
                  ...
                  className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded text-xs flex items-center gap-1"
                ...
                <button
                  ...
                  className="px-3 py-2 bg-orange-600 hover:bg-orange-700 text-white rounded text-xs flex items-center gap-1"
                ...
                <button
                  ...
                  className="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded text-xs flex items-center gap-1"
                ...
                <button
                  onClick={() => {
                    const debug = (window as any).__isolationDebug
                    if (debug) {
                      console.log('=== Isolation Debug Info ===')
                      console.log('Isolated IDs:', debug.list())
                      console.log('FPS:', debug.getFps ? debug.getFps() : 'N/A')
                      console.log('Enabled:', isolationEnabled)
                      console.log('Config:', isolationConfig)
                    }
                  }}
                  className="px-3 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded text-xs flex items-center gap-1"
            ...
            {isolatedIds.length > 0 && (
              <div className="bg-gray-800 rounded p-3">
                <div className="text-sm text-gray-400 mb-2">Active Isolations</div>
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {isolatedIds.map(id => (
                    <div key={id} className="flex items-center justify-between py-1 px-2 bg-gray-700 rounded text-xs">
                      <span className="text-yellow-400 font-mono truncate">{id}</span>
                      <button
                        onClick={() => {
                          const debug = (window as any).__isolationDebug
                          if (debug) {
            ...
            <div className="bg-black rounded p-3">
              <div className="text-xs text-gray-500 mb-2">Console Output (Open DevTools)</div>
