...
export function CanvasPanel({ panelId, branch, position, onClose, noteId }: CanvasPanelProps) {
  ...
  const panelHoverTimeoutRef = useRef<NodeJS.Timeout>()
      ...
      >
        ...
        <div style={{ 
          ...
          <button
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.background = "rgba(255,255,255,0.2)"
              // Hide sidebar after a delay when mouse leaves
          ...
          {/* Actions button (Note/Explore/Promote) */}
          <button
            ...
            onMouseEnter={(e) => {
              e.currentTarget.style.background = "rgba(255,255,255,0.3)"
              // Show annotation toolbar on hover
              if (actionsTimeoutRef.current) {
                clearTimeout(actionsTimeoutRef.current)
              }
              setIsActionsHovering(true)
              
              // Position the annotation toolbar below the Actions button
              const toolbar = document.getElementById("annotation-toolbar")
              if (toolbar) {
                const rect = e.currentTarget.getBoundingClientRect()
                toolbar.style.position = "fixed"
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.background = "rgba(255,255,255,0.2)"
              // Hide annotation toolbar after a delay when mouse leaves
              actionsTimeoutRef.current = setTimeout(() => {
                setIsActionsHovering(false)
                const toolbar = document.getElementById("annotation-toolbar")
                // Only hide if not hovering over the toolbar
        ...
        {/* Spacer between buttons and drag icon - explicit for all panels */}
        <div style={{ 
          width: '12px',
          minWidth: '12px',
          height: '24px',
        ...
        {/* Drag Handle Indicator - Shows with buttons, or alone as a hint */}
        <div
          ...
          style={{
            ...
            transition: 'opacity 0.2s ease',
            color: 'white',
            // Show when buttons are visible, or show faintly when nothing is hovered
            opacity: (isPanelHovered || isSidebarVisible || isActionsVisible || isSidebarHovering || isActionsHovering) 
              ? 1  // Full opacity when buttons are visible
              : 0.5, // Half opacity as a hint when idle
