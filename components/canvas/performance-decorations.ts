...
interface PerformanceMetrics {
  ...
  clickCount: number
  tooltipShown: number
  lastYjsSync: number | null
  decorationUpdates: number
}
  ...
  return new Plugin({
    ...
    state: {
      ...
      apply(tr, value) {
        ...
        metrics.decorationUpdates++
        updatePerformanceIndicator()
        
        return { metricsUpdated: Date.now() }
      }
    },
    ...
    props: {
      handleDOMEvents: {
        mouseover(view, event) {
          ...
          if (target.closest('.annotation-hover-target')) {
            metrics.hoverCount++
            updatePerformanceIndicator()
          }
          return false
...
// Toggle performance indicator with keyboard shortcut
if (typeof window !== 'undefined') {
  window.addEventListener('keydown', (e) => {
    if (e.ctrlKey && e.shiftKey && e.key === 'P') {
      if (indicatorElement) {
        indicatorElement.style.display = 
          indicatorElement.style.display === 'none' ? 'block' : 'none'
      }
    }
  })
}
