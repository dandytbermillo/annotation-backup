...
export function FloatingToolbar({ x, y, onClose, onSelectNote, onCreateNote, onCreateOverlayPopup, onAddComponent, editorRef, activePanelId, onBackdropStyleChange, onFolderRenamed, activePanel: activePanelProp, onActivePanelChange, refreshRecentNotes, onToggleConstellationPanel, showConstellationPanel, canvasState, canvasDispatch, canvasDataStore, canvasNoteId, knowledgeBaseWorkspace, workspaceReady = true, workspaceName = null }: FloatingToolbarProps) {
  ...
  const handleSelectFolderColor = async (folderId: string, newColorName: string, currentColor: string | undefined) => {
    ...
    // Update database
    try {
      const response = await fetchWithKnowledgeBase(`/api/items/${folderId}`, {
        method: 'PATCH',
        headers: {
  ...
  const handleCreateFolderInOrg = async () => {
    ...
    try {
      ...
      const treeResponse = await fetchWithKnowledgeBase(treeUrl)
      if (treeResponse.ok) {
        ...
        if (knowledgeBase) {
          ...
          const childResponse = await fetchWithKnowledgeBase(childUrl)
          if (childResponse.ok) {
            const childData = await childResponse.json()
            const children = childData.items || []
  ...
  const handleSaveRenameFolder = async () => {
    ...
    try {
      ...
      const response = await fetchWithKnowledgeBase(`/api/items/${renamingFolderId}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
        },
