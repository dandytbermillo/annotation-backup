...
function validateInitial(featureSlug, config) {
  ...
  for (const section of requiredSections) {
    ...
    // For other sections, use consistent camelCase normalization
    let sectionKey = section;
    
    // Map the required section name to what's actually stored in sections object
    if (section === 'acceptanceCriteria') {
      // The parser stores this as 'acceptanceCriteria' (camelCase)
      sectionKey = 'acceptanceCriteria';
    } else {
      // For other sections, check what the parser actually stored
      sectionKey = section.toLowerCase().replace(/\s+/g, '');
    }
