...
# Modified to use Docker for PostgreSQL commands
...
cleanup_db() {
    ...
    psql_docker -v ON_ERROR_STOP=1 -c "DELETE FROM document_saves WHERE note_id IN (SELECT id FROM notes WHERE title LIKE 'Fix % Test%' OR title LIKE 'Performance Test' OR title LIKE 'Concurrent Test' OR title LIKE 'Large Doc Test' OR title LIKE 'Note %' OR title LIKE 'State test%');" 2>/dev/null || true
    psql_docker -v ON_ERROR_STOP=1 -c "DELETE FROM branches WHERE note_id IN (SELECT id FROM notes WHERE title LIKE 'Fix % Test%' OR title LIKE 'Performance Test' OR title LIKE 'Concurrent Test' OR title LIKE 'Large Doc Test' OR title LIKE 'Note %' OR title LIKE 'State test%');" 2>/dev/null || true
    psql_docker -v ON_ERROR_STOP=1 -c "DELETE FROM notes WHERE title LIKE 'Fix % Test%' OR title LIKE 'Performance Test' OR title LIKE 'Concurrent Test' OR title LIKE 'Large Doc Test' OR title LIKE 'Note %' OR title LIKE 'State test%';" 2>/dev/null || true
    log_success "DB cleanup complete"
}

# Set up trap for cleanup (without notify script since it doesn't exist)
trap 'status=$?; cleanup; cleanup_db;' EXIT
...
check_prerequisites() {
    ...
    # Check PostgreSQL connection via Docker
    if ! psql_docker -c "SELECT 1" &> /dev/null; then
        log_error "Cannot connect to PostgreSQL in Docker container"
        exit 1
    fi
    
    # Check if migrations are applied
    if ! psql_docker -c "SELECT 1 FROM document_saves LIMIT 1" &> /dev/null; then
...
verify_storage_format() {
    ...
    local binary_check=$(psql_docker -t -c "
        SELECT COUNT(*) 
        FROM document_saves 
...
check_yjs_artifacts() {
    ...
    local yjs_patterns=$(psql_docker -t -c "
        SELECT COUNT(*) 
        FROM document_saves 
    ...
    local yjs_tables=$(psql_docker -t -c "
        SELECT COUNT(*) 
...
test_panel_id_normalization() {
    ...
    local db_check=$(psql_docker -t -c "SELECT panel_id FROM document_saves WHERE note_id = '$note_id' LIMIT 1" 2>/dev/null | xargs)
