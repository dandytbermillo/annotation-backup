...
const ModernAnnotationCanvasInner = forwardRef<CanvasImperativeHandle, ModernAnnotationCanvasProps>(({ 
        ...
        prev.map(item => {
          if (item.itemType === 'panel' && item.panelId === 'main') {
            ...
            if (itemNoteId === targetNoteId) {
              return { ...item, position: workspaceMainForTarget }
            }
          }
          return item
        })
  ...
  }, [buildHydratedPanels, canvasItems, dispatch, getItemNoteId, resolveWorkspacePosition, freshNoteSet, onFreshNoteHydrated, setCanvasItems, initialCanvasSetupRef, mergeHydratedPanels])
