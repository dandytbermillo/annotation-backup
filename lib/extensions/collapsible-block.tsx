...
export const CollapsibleBlock = Node.create({
  ...
  addKeyboardShortcuts() {
    const guardBackspace = ({ editor }: { editor: any }) => {
      ...
      const paragraphType = schema.nodes.paragraph
      const collapsibleType = schema.nodes.collapsibleBlock
      if (!selection.empty || !paragraphType || !collapsibleType) {
        return false
      }
      ...
      const resolvedParent = state.doc.resolve($from.before())
      const prevNode = resolvedParent.nodeBefore
      if (!prevNode || prevNode.type !== collapsibleType) {
        return false
      }

      const anchorPos = $from.pos
      const tr = state.tr.setSelection(TextSelection.near(state.doc.resolve(anchorPos)))
    ...
    return {
      ...
      'Shift-Backspace': guardBackspace,
    }
  },
...
function CollapsibleBlockFull({ node, updateAttributes, editor, getPos }: any) {
  ...
  const handleHeaderMouseDownCapture = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (event.shiftKey) {
      ...
      if (snapshot && alreadySelected && (snapshot.mode === 'range' || snapshot.mode === 'multi')) {
        logCollapsibleSelectionDebug('NODEVIEW_SHIFT_MOUSEDOWN_SKIP_EXISTING', {
          nodePos,
          selectionPos,
          snapshot,
        })
      ...
      if (commandPos != null) {
        ...
        event.stopPropagation()
        editor?.view?.focus()
        const applied = editor?.commands.setCollapsibleBlockRange(commandPos) ?? false
        void debugLog('CollapsibleBlockNodeView', applied ? 'shift_set_range_deferred' : 'shift_set_range_deferred_failed', {
          metadata: {
            nodePos,
            selectionPos,
            commandPos,
    ...
    if (event.metaKey || event.ctrlKey) {
      const selectionSnapshot = (editor?.storage as any)?.collapsibleBlockSelection?.snapshot ?? null
      logCollapsibleSelectionDebug('NODEVIEW_META_MOUSEDOWN_DEFER', {
        nodePos,
        selectionPos,
        snapshot: selectionSnapshot,
        modifiers: {
      ...
      if (commandPos != null) {
        ...
        event.stopPropagation()
        editor?.view?.focus()
        const applied = editor?.commands.toggleCollapsibleBlockSelection(commandPos) ?? false
        void debugLog('CollapsibleBlockNodeView', applied ? 'meta_toggle_selection_deferred' : 'meta_toggle_selection_deferred_failed', {
          metadata: {
            nodePos,
            selectionPos,
            commandPos,
  ...
  const handleHeaderClickCapture = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (hasModifierKey(event)) {
      shouldEditTitleOnClickRef.current = false
      logCollapsibleSelectionDebug('NODEVIEW_MOD_CLICK_CAPTURE_DEFER', {
        modifiers: {
          shift: event.shiftKey,
          meta: event.metaKey,
          ctrl: event.ctrlKey,
    ...
    const shouldEditTitle = shouldEditTitleOnClickRef.current
    shouldEditTitleOnClickRef.current = false
  ...
  const handleWrapperMouseDownCapture = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (target?.closest('[data-multi-selection-actions]') || target?.closest('button')) {
      return
    }
          ...
          <>
            ...
            >
              ...
              >
                ...
                <button type="button" onClick={(e) => { e.stopPropagation(); handleMove('down') }} style={{ border: 'none', background: 'none', padding: 0, cursor: 'pointer', color: '#708090', fontSize: '14px' }} aria-label="Move block down">▼</button>
                <button type="button" onClick={(e) => { handleOpenInspector(e) }} style={{ border: 'none', background: 'none', padding: 0, cursor: 'pointer', color: '#708090', fontSize: '14px' }} aria-label="Open block inspector">🗔</button>
                <button type="button" onClick={(e) => { e.stopPropagation(); handleDuplicate() }} style={{ border: 'none', background: 'none', padding: 0, cursor: 'pointer', color: '#708090', fontSize: '14px' }} aria-label="Duplicate block">⧉</button>
                <button type="button" onClick={(e) => { e.stopPropagation(); handleDelete() }} style={{ border: 'none', background: 'none', padding: 0, cursor: 'pointer', color: '#cc4a4a', fontSize: '14px' }} aria-label="Delete block">🗑</button>
              </div>
            </div>
          </>
        )}
