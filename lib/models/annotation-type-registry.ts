...
export class AnnotationTypeRegistry {
   ...
   * 2. Reloads fresh data from database (BLOCKS until complete)
   * 3. Notifies all subscribers AFTER reload completes
   *
   * @returns Promise that resolves when cache is reloaded
   *
   * @example
   * // After creating a new type via POST
   * await pool.query('INSERT INTO annotation_types...');
   * await registry.invalidate(); // Blocks until registry is fresh
   * // Now registry.getAll() will include the new type
   */
  async invalidate(): Promise<void> {
    console.log('[AnnotationTypeRegistry] Invalidating cache...');

    // 1. Clear current cache explicitly (defensive)
    this.cache.clear();
    this.loaded = false;
    this.loadPromise = null;

    // 2. Reload from DB (blocks until complete)
    await this.ensureLoaded();

    // 3. Notify all subscribers AFTER reload completes
    this.notify();
