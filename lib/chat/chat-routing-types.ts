...
export interface EscapeEvidence {
  // B1 removed (Slice B3) — semantic metadata subsumes exact-memory
  semantic?: {
...
export interface PanelDisambiguationHandlerResult extends HandlerResult {
  ...
  matchCount?: number
  /** Message ID of the clarifier (multi-match case only). Used by Phase 3c telemetry. */
  clarifierMessageId?: string
  /** Matched panel candidates as {id, label, type} for Phase 3c telemetry. */
